...
public final class Util {
  ...
  public static byte[] executePost(String url, byte[] data, Map<String, String> requestProperties)
    ...
    Log.d("PlayerActivity","executePost");
    ...
    try {
      ...
      Map<String, List<String>> getHeader = urlConnection.getHeaderFields();
      for (Map.Entry<String, List<String>> entry : getHeader.entrySet()) {
        String Key = entry.getKey();
        if (Key == null) {
        } else if (Key.contains("X-Lifemedia-Quality")) {
          responseHeader = entry.getValue().get(0);
        }
      }
      if (responseHeader != "" && ContextInformation.contextEnable) {
        ContextInformation.responseQuality = Integer.parseInt(responseHeader);
      } else {
        ContextInformation.responseQuality = Integer.parseInt("0");
      }
      try {
